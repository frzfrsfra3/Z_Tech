  ...
  csvQueue.process(async (job) => {
    try {
      ...
      if (type === "batch") {
        ...
        const bulkOps = records.map((record: { name: string; email: string }) => ({
          insertOne: {
            document: {
              name: record.name,
              email: record.email,
      ...
      if (job.data.type === "batch") {
        const failedRecords = job.data.records.filter((_: any, i: number) => 
          error.writeErrors?.some((e: any) => e.index === i)
        );
