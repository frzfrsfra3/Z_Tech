...
import { PropertyCSVRow } from '../types/property.types.js'; // Add .js extension
export const importPropertiesCSV = async (req: Request, res: Response) => {
  if (!req.file) {
  ...
  try {
    ...
    stream.on('data', async (row: PropertyCSVRow) => {
      totalRows++;
